...
const JobDetailEmployerScreen = ({ route, navigation }: any) => {
    ...
    const handleSave = async () => {
        try {
            const jobQuerySnapshot = await fbJobDetail.where('sMaTinTuyenDung', '==', sMaTinTuyenDung).get();
            if (!jobQuerySnapshot.empty) {
                const jobDocId = jobQuerySnapshot.docs[0].id;

                await fbJobDetail.doc(jobDocId).update({
                    sCoKhoa: 3
                });
    ...
    const menuOptions = [
        {
            label: "Chỉnh sửa tin",
            onPress: () => navigation.navigate("edit-job", { sMaTinTuyenDung }),
        },
        {
            label: "Khóa tin",
    ...
    return (
        <View style={[styles.wrapper, { backgroundColor: theme.bG }]}>
            <HeaderWithIcons
                ...
                rightIcon={require('../../../../asset/images/img_menu.png')}
                onRightPress={toggleMenu}
            />
        <MenuEdit
            visible={menuVisible} 
            onClose={toggleMenu} 
...
export default JobDetailEmployerScreen;
